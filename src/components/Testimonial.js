...
const Testimonial = () => {
  ...
  const getReviews = async () => {
    const response = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/reviews?populate=*`,);
  ...
  return (
    <>
      <div className={`${styles.testimonial}`}>
        ...
        <span className={styles.subhead}>Testimonials</span>
              ...
              return <SwiperSlide key={index} className='d-flex justify-content-center align-items-center'>
                <div className={styles.slide}>
                  ...
                  <div className={styles.user}>
                    {item.attributes.image ? <Image width={100} height={60} src={imageUrl} alt={item.attributes.name} className={index == 0 ? styles.img1 : null} />
